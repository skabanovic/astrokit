...
def optical_depth(vel, vel_0, width, temp_ex, col_den):

    temp_0 = (const.plank * line.freq_0)/const.boltzmann

    tau = line.profile_norm(vel, vel_0, width) * \
          ((line.wei_up * line.einstein_coef * const.speed_light**3) /\
           (line.wei_low * 8. * np.pi * line.freq_0**3)) *\
          col_den * 1.0e4 * (1. - np.exp(-temp_0/temp_ex)) /\
          (1. + (line.wei_up/line.wei_low) * np.exp(-temp_0/temp_ex))
...
def two_layer(bakg_num, forg_num, vel, *param):
    ...
    if (forg_num > 0):
        ...
        for i in range(0, (4*bakg_num), 4):
            ...
            bakg_sum += brightness_temperatur(temp_ex)*(1. - np.exp(-optical_depth(vel, vel_0, width, temp_ex, col_den)))
        ...
        for i in range((4*bakg_num), (4*bakg_num+4*forg_num), 4):
            ...
            forg_tau = forg_tau + optical_depth(vel, vel_0, width, temp_ex, col_den)

            forg_sum = forg_sum +\
                       brightness_temperatur(temp_ex)*(1.-np.exp(-optical_depth(vel, vel_0, width, temp_ex, col_den)))
